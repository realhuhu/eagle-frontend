<template>
  <div class="p-4">
    <div class="md:flex justify-between items-start gap-6">
      <a-form-item label="排序" class="md:w-[40%]">
        <a-select placeholder="请选择" v-model="params.order">
          ...
          <a-option value="-chat_price">价值降序</a-option>
          <a-option value="chat_price">价值升序</a-option>
        </a-select>
      </a-form-item>
    ...
    <div class="md:flex justify-between items-start gap-6">
      <div class="md:flex justify-between items-start gap-6 md:w-full">
        <a-form-item label="搜索">
          <div class="flex justify-start items-center md:w-[70%] gap-6">
            ...
            <a-input placeholder="搜索内容" v-model="params.search" allow-clear/>
          </div>
        </a-form-item>
    ...
    <transition enter-active-class="animate__animated animate__fadeIn" appear>
      ...
      <div v-else>
        <div v-for="(chat,k) of data.chat" :key="k">
          <transition enter-active-class="animate__animated animate__fadeIn" appear>
            <chat-record :chat="chat"/>
          </transition>
        </div>
...
import ChatRecord from "@/components/record/ChatRecord.vue";
...
const data = ref<{ count: number, chat: Chat[] }>({
  count: 0,
  chat: []
})
...
const get_data = async () => {
  try {
    loading.value = true
    const res = await client.get<PaginatedResponse<Chat>>({
      url: "chat",
      params: {
        ...params.value,
    ...
    if (res.code === 100) {
      ...
      data.value.chat = res.data.results
    } else {
