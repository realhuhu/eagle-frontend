<template>
  <div class="p-4">
    <div class="md:flex justify-between items-start gap-6">
      ...
      <a-form-item class="md:w-[60%]" label="上舰">
    ...
    <transition enter-active-class="animate__animated animate__fadeIn" appear>
      ...
      <div v-else>
        <div v-for="(entry,k) of data.entry" :key="k">
          <transition enter-active-class="animate__animated animate__fadeIn" appear>
...
const data = ref<{ count: number, entry: Entry[] }>({
  count: 0,
  entry: []
})
...
const get_data = async () => {
  try {
    loading.value = true
    const res = await client.get<PaginatedResponse<Entry>>({
      url: "entry",
      params: {
        ...params.value,
    ...
    if (res.code === 100) {
      ...
      data.value.entry = res.data.results
    } else {
