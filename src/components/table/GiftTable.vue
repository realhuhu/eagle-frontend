  ...
  <div class="p-4">
    <div class="md:flex justify-between items-start gap-6">
      <a-form-item label="类型">
        <a-select v-model="params.coin_type" placeholder="请选择..." multiple>
          <a-option :value="0" :tag-props="{color:'gray'}">银瓜子</a-option>
          <a-option :value="1" :tag-props="{color:'gold'}">金瓜子</a-option>
        </a-select>
      </a-form-item>

      <a-form-item label="舰长">
      ...
      <div class="flex justify-between items-start gap-6 md:w-full">
        ...
        <a-form-item label="排序">
          <a-select placeholder="请选择" v-model="params.order">
            ...
            <a-option value="-gift_total_price">价值降序</a-option>
            <a-option value="gift_total_price">价值升序</a-option>
          </a-select>
        </a-form-item>
      </div>
    ...
    <transition enter-active-class="animate__animated animate__fadeIn" appear>
      ...
      <div v-else>
        <div v-for="(gift,k) of data.gift" :key="k">
          <transition enter-active-class="animate__animated animate__fadeIn" appear>
            <gift-record :gift="gift"/>
          </transition>
        </div>
...
const params = ref<Params<{ coin_type: number[] }>>({
  page: 1,
  size: 20,
  order: "-timestamp",
  coin_type: [0, 1],
...
const data = ref<{ count: number, gift: Gift[] }>({
  count: 0,
  gift: []
})
...
const get_data = async () => {
  try {
    ...
    if (res.code === 100) {
      ...
      data.value.gift = res.data.results
    } else {
